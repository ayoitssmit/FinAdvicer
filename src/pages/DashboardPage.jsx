...
// --- Item Modal Component (Handles both Add and Edit) ---
...
export default function DashboardPage() {
    ...
    const handleRefreshPrices = async () => {
        ...
        try {
            for (const stock of updatedData.stocks) {
                const res = await fetch(`${API_BASE_URL}/stock/${stock.name}`);
                if (res.ok) { stock.currentPrice = (await res.json()).price; pricesUpdated = true; }
            }
            if (updatedData.gold.length > 0) {
                const res = await fetch(`${API_BASE_URL}/commodities/gold`);
                if (res.ok) { const price = (await res.json()).price_per_gram_inr; updatedData.gold.forEach(i => i.currentValue = price); pricesUpdated = true; }
            }
            if (updatedData.silver.length > 0) {
                const res = await fetch(`${API_BASE_URL}/commodities/silver`);
                if (res.ok) { const price = (await res.json()).price_per_gram_inr; updatedData.silver.forEach(i => i.currentValue = price); pricesUpdated = true; }
            }
            if (pricesUpdated) setFinancialData(updatedData);
        ...
        switch (activeCategory) {
            ...
            case 'mutualFunds':
                ...
                total = items.reduce((acc, m) => {
                    const totalInvested = m.invested * (Math.floor(((new Date() - new Date(m.startDate)) / (1000 * 60 * 60 * 24 * 30.44)) / m.recurringMonths) + 1);
            ...
            default:
                const categoryInfo = categories.lifeEvents.find(c => c.id === activeCategory);
                label = `Total ${categoryInfo ? categoryInfo.label : ''} Cost`;
    ...
    const renderItem = (category, item) => {
        ...
        switch (category) {
            ...
            case 'mutualFunds':
                ...
                const totalInvested = item.invested * (Math.floor(((new Date() - new Date(item.startDate)) / (1000 * 60 * 60 * 24 * 30.44)) / item.recurringMonths) + 1);
            ...
            case 'gold':
            case 'silver': profitLoss = item.currentValue - item.purchasePrice; isItemProfit = profitLoss >= 0;
                return <><span className="item-name">{item.name}</span> <span className={isItemProfit ? 'text-profit' : 'text-loss'}>Value Gain: ${profitLoss.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></>;
            case 'postRetirement':
    ...
    const renderFooter = () => {
        const { amount, type, label } = categoryTotal;
        const sign = type === 'cost' ? '-' : (amount >= 0 ? '+' : '-');
        const className = `total-amount ${type === 'cost' || amount < 0 ? 'text-loss' : 'text-profit'}`;
                    ...
                    <ul className="category-list">
                        {categories.investment.map(({ id, label }) => (
                            <li key={id} className="category-item">
                                ...
                                <button onClick={() => openModal('add', id)} className="add-item-button">
                                    <PlusIcon />
                                </button>
                            </li>
                        ))}
                                ...
                                financialData[activeCategory].map(item => (
                                    <li key={item.id} className="item-list-entry">
                                        ...
                                        <div className="item-actions">
                                            <button onClick={() => openModal('edit', activeCategory, item)} className="edit-item-button">
                                                <EditIcon />
                                            </button>
                                            <button onClick={() => handleRemoveItem(activeCategory, item.id)} className="remove-item-button">
                                                <TrashIcon />
                                            </button>
                                        </div>
                                    </li>
